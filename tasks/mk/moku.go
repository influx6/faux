...
func init(){
  ...
  RootDirectory.Set("/",func() *VDir{
    ...
    // register the files
		...
		dir.AddFile(NewVFile("/home/alex/local/cmd/src/github.com/influx6/faux/tasks","tasks.go","tasks.go",6774,false,true,func(v *VFile) ([]byte, error) {
		...
		dir.AddFile(NewVFile("/home/alex/local/cmd/src/github.com/influx6/faux/tasks","tasks_test.go","tasks_test.go",3407,false,true,func(v *VFile) ([]byte, error) {
			fo, err := os.Open(v.RealPath())
			if err != nil {
				return nil, fmt.Errorf("---> assets.readFile: Error reading file: %s at %s: %s\n", v.Name(), v.RealPath(), err)
			}
